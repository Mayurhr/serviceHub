...
const avatarColor = (name = '') => {
  const colors = ['#6366f1','#f97316','#10b981','#3b82f6','#8b5cf6','#ec4899','#14b8a6','#f59e0b'];
...
export default function ProviderCard({ provider }) {
  ...
  // Use data URI avatar from backend, or generate initials locally
  const hasValidAvatar = provider.avatar && (provider.avatar.startsWith('data:') || provider.avatar.startsWith('https://images.'));
          ...
          <div className="relative flex-shrink-0">
            {hasValidAvatar ? (
              <img src={provider.avatar} alt={provider.name}
                className="w-14 h-14 rounded-2xl object-cover ring-2 ring-ink-100 group-hover:ring-saffron-200 transition-all"
                onError={e => { e.target.style.display = 'none'; e.target.nextSibling.style.display = 'flex'; }} />
            ) : null}
            <div className={`w-14 h-14 rounded-2xl flex items-center justify-center text-white font-extrabold text-lg ring-2 ring-ink-100 group-hover:ring-saffron-200 transition-all ${hasValidAvatar ? 'hidden' : ''}`}

...
const Guard = ({ children, adminOnly = false, providerOnly = false, noAdmin = false }) => {
  const { user, isAdmin, isProvider } = useAuth();
  if (!user) return <Navigate to="/login" />;
  if (adminOnly && !isAdmin) return <Navigate to="/" />;
  if (noAdmin && isAdmin) return <Navigate to="/admin" />;
  ...
  <div className="min-h-screen flex flex-col">
    ...
    <main className="flex-1">
      <Routes>
        ...
        <Route path="/book/:slug" element={<Guard noAdmin><BookingPage /></Guard>} />
        ...
        <Route path="/provider/chat/:bookingId" element={<Guard providerOnly><ProviderChatPage /></Guard>} />
